			...
			foreach($list as $r)
			{
				...
				if(!empty($d) && !empty($d['packages'])) {
					foreach($d['packages'] as $pkg=>$ver) {
						...
						$p[$pkg]['category']=!empty($ver[$v[0]]['keywords_'.PHPPE::$client->lang][0])?$ver[$v[0]]['keywords_'.PHPPE::$client->lang][0]:(!empty($ver[$v[0]]['keywords_en'][0])?$ver[$v[0]]['keywords_en']:(!empty($ver[$v[0]]['keywords'][0])?$ver[$v[0]]['keywords'][0]:""));
