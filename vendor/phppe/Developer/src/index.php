 ...
 */
	class AddOn
	{
		...
		protected $css;				//!< css class to use, input or reqinput
		protected $err;				//!< appended to output on error (empty or html sniplet)
/**
 ...
 */
		final function __construct($a, $n, &$v, $t = [], $r = 0)
		{
			...
			$this->css = (! empty($r) ? "req" : "") . "input";
			//! mark failed fields with background color
			//! you should not use inline background in your addon's output,
			//! always use css class to specify background
			$this->err = Core::iserror($n) ? " style='background:" . Core::$ec . ";'" : "";
		}
/**
 ...
 */
	class Core
	{
		...
		public static $ec = "#FFA0A0";		//!< default error background color
		...
		private static $sel;			//!< data source selector
 ...
 */
		function __construct($islib = true)
		{
			...
			{
				self::log('C', "Uncought exception: " . $e->getMessage()); 
 ...
 */
		private function bootdiag()
		{
			...
			$D = [ ".tmp" => 0770, "data" => 0770, "app" => 0, M => 0, M . "bin" => 0, N => 0, P => 0, P . "log" => 0770, P . "views" => 0, P . "ctrl" => 0, P . "lang" => 0, P . "libs" => 0, P . "out" => 0, P . "sql" => 0, "public/images" => 0, "public/css" => 0, "public/js" => 0, "app/ctrl" => 0, "app/lang" => 0, "app/libs" => 0, "app/views" => 0 ];
			foreach([ "*", "*/*", "*/*/*", "*/*/*/*" ] as $v)
				$D += array_fill_keys(@glob(M . $v, GLOB_ONLYDIR), 0);
			foreach($D as $d => $p) {
				if(! $p)
					$p = $C;
				if(! is_dir($d)) {
 ...
 */
		private function bootstrap()
		{
			...
			if(self::$w) {
				...
				$L = "pe_tz";
				if($this->app == "index" && empty($_SESSION[ $L ]) && ! isset($R[ 'nojs' ])) {
			...
			if(f($c)) {
				...
				$this->js[ "L(t)" ] = "return t.replace(/_/g,' ');";
			...
			if(! empty($this->db)) {
				...
				//! this will override time() in $core->now
				$d = "CURRENT_TIMESTAMP";
				try {
 ...
 */
		function run($n = "", $ac = "")
		{
			...
			if(in_array($this->app, [ "css", "js", "images" ])) {
				...
				$N = 'a_' . sha1($this->base . $u . "_" . self::$user->id . "_" . self::$client->lang);
				$d = "";
				if(! empty(self::$mc))
					$d = self::$mc->get($N);
				if(! empty($d))
				...
				if(! empty(Core::db(0)) && $G) {
					try {
						...
						if(! empty($D[ $T ])) {
							...
							$o = @jd($D[ 'data' ]);
							if(! a($o))
								$o = [];//! get extra parameters from url
							$c = w($u, u($D[ 'id' ]) + 1);
							$o[ 'params' ] = x("/", $c);
						}
					}
			...
			if(empty($this->$m) || ! self::$w) {
				...
				$C = 'p_' . sha1($this->base . $u . "_" . self::$user->id . "_" . self::$client->lang);
				$T = "";
				...
				if(! $T) {
					...
					if(! empty($app->meta))
						$this->meta = array_merge($this->meta, $app->meta);
					//! clear validators
					$_SESSION[ $V ] = [];
						...
						if(! empty($this->menu)) {
							foreach($this->menu as $e => $L) {
								...
								else
								{
									...
									if(empty($d[ 0 ]))
										$d = array_shift($d);
									if($this->app == (! empty($d[ 0 ]) ? $d[ 0 ] : "index"))
										$a = 1;
									unset($d);
								}
								if(a($L)) {
 ...
 */
		static function log($w, $m, $n = null)
		{
			...
			if(! in_array($w, [ "D", "I", "A", "W", "E", "C" ]))
				$w = "A";
			if(empty($n))
				$n = ! empty(self::$core->app) ? self::$core->app : "phppe";
			$m = r($m);
 ...
 */
		static function lib($n = "", $l = "", $D = "", &$o = null)
		{
			...
			if($l == "" && empty($D) && ! $o)
				return empty($n) ? $L : $L[ $n ];//! initialize a module with dependency check
			$d = "";
 ...
 */
		static function url($m = "", $p = "")
		{
			...
			return "http" . (self::$core->sec ? "s" : "") . "://" . $c . ($c[ u($c) - 1 ] != "/" ? "/" : "") . ($f != "index" . PE ? $f . ($a ? "/" : "") : "") . $a;
		}
/**
 * redirect user
 *
 ...
 * @return selector for this new data source
 */
		static function dbInit($u, $O = null)
		{
			//initialize a database and make connection available as a data source
			$S = microtime(1);
			//create instance
			try {
				...
				self::log(self::$sel ? "E" : "C", "Unable to init: $u, " . $e->getMessage(), "phppe");
				throw $e;
			}
...
/**
 * return database instance for a data source selector
 *
 * @usage if you want to do further optimalisation on database queries, like pre-parsed sql or to avoid fetchAll()
 *
 * @param data source selector
 * @return instance
 */
		static function db($s = - 1)
		{
			//return database instance for a specific data source selector
			if($s == - 1)
				$s = self::$sel;
			if($s >= 0 && $s < count(self::$db) && ! empty(self::$db[ $s ]))
				return self::$db[ $s ];
...
/**
 * execute a query on current data source
 *
 * @param query string
 ...
 */
		static function css($c = "")
		{
			...
			if(in_array(m($a), [ "ctrl", "libs", "js", "css", "addons" ]))
				$a = n($a);
			$a .= "/css/" . $c;
			if(! f($a))
				$a .= ".php";
		...
		static function getval($x)
		{
			...
			for($i = 0; $i < u($d); $i++ ) {
				...
				if((ctype_alpha($c) || $c == "_") && ! ctype_alnum($l) && $l != "." && $l != "_") {
					...
					elseif($b == "(" && ($j - $i != 1 || $d[ $i ] != "L") && z($d, $i, 5) != "core." && z($d, $i, $j - $i) != "array" && ! empty(self::$core->allowed) && ! in_array(z($d, $i, $j - $i), self::$core->allowed))
						...
						if(p("/^([^\(]+)[\(]?([^\)]*)/", $A[ 0 ], $ma) && ! empty($ma[ 1 ])) {
							...
							if(ce($d) && is_subclass_of($d, C . "AddOn")) {
								...
								if($R || $f == "check" || $f == "file" || q(A . $f, "validate"))
									$_SESSION[ "pe_v" ][ $n ][ $f ] = [ $R, $a, $A ];//find out method to use to draw AddOn
 ...
 */
	class button extends X
	{
		...
		function edit()
		{
			return "<input class='" . (! empty($this->attrs[ 1 ]) && $this->attrs[ 1 ] != "-" ? $this->attrs[ 1 ] : "button") . "' type='button' value=\"" . h(L(! empty($this->name) ? $this->name : "Press me")) . "\" onclick=\"" . s(! empty($this->attrs[ 0 ]) && $this->attrs[ 0 ] != "-" ? $this->attrs[ 0 ] : "alert('" . L("No action") . "');", "\"", "\\\"") . "\">";
		}
	}
 ...
 */
	class text extends X
	{
		function edit()
		{
			...
			if($v == "null")
				$v = "";
			if(! empty($this->args[ 2 ]) && $this->args[ 2 ] > 0) {
				if($this->args[ 1 ] > 0)
 ...
 */
	class select extends X
	{
		...
		function edit()
		{
			$opts = ! empty($this->attrs[ 0 ]) && $this->attrs[ 0 ] != "-" ? Core::getval($this->attrs[ 0 ]) : [];
			if(is_string($opts))
				$opts = x(",", $opts);
			$skip = ! empty($this->attrs[ 1 ]) && $this->attrs[ 1 ] != "-" ? Core::getval($this->attrs[ 1 ]) : [];
			if(is_string($skip))
				$skip = x(",", $skip);
			if(! a($skip))
				$skip = [];
			...
			$r = "<select" . @v($this->css, $this->attrs[ 3 ], $this->err, $this->name . (! empty($this->args[ 1 ]) ? "[]" : ""), $this->attrs[ 2 ]) . (! empty($this->args[ 1 ]) ? " multiple" : "") . (! empty($this->args[ 0 ]) && $this->args[ 0 ] > 0 ? " size='" . intval($this->args[ 0 ]) . "'" : "") . " onfocus='this.style.background=\"\";'>";
			if(a($opts))
				foreach($opts as $k => $v) {
					...
					if(! isset($skip[ $o ]) && ! empty($n))
						$r .= "<option value=\"" . h($o) . "\"" . ((a($this->value) && in_array($o . "", $this->value)) || $o == $this->value ? " selected" : "") . ">" . $n . "</option>";
					}
			$r .= "</select>";
			return $r;
		}
	...
	class check extends X
	{
		function show()
		{
			if(empty(Core::$core->output) || Core::$core->output != "html")
				return "[" . (! empty($this->value) ? "X" : " ") . "]" . (! empty($this->attrs[ 0 ]) ? L($this->attrs[ 0 ]) : $this->value);
			return h($this->value);
		}
		function edit()
		{
			return(! empty($this->err) ? "<span " . $this->err . ">" : "") . "<input" . @v($this->css, $this->attrs[ 2 ], "", $this->name, $this->attrs[ 1 ]) . " id='" . $this->name . "' type='checkbox'" . (! empty($this->value) ? " checked" : "") . " value=\"" . h(r(! empty($this->args[ 0 ]) ? $this->args[ 0 ] : "1")) . "\">" . (! empty($this->attrs[ 0 ]) ? "<label for='" . $this->name . "'>" . L($this->attrs[ 0 ]) . "</label>" : "") . (! empty($this->err) ? "</span>" : "");
		}
	}
 ...
 */
	class radio extends X
	{
		...
		function edit()
		{
			return "<input" . @v($this->css, $this->attrs[ 2 ], $this->err, $this->name, $this->attrs[ 1 ]) . " id='" . $this->name . "_" . sha1($this->args[ 0 ]) . "' type='radio'" . ($this->value == $this->args[ 0 ] ? " checked" : "") . " value=\"" . h(r(! empty($this->args[ 0 ]) ? $this->args[ 0 ] : "1")) . "\">" . (! empty($this->attrs[ 0 ]) ? "<label for='" . $this->name . "_" . sha1($this->args[ 0 ]) . "'>" . L($this->attrs[ 0 ]) . "</label>" : "");
		}
	}
 ...
 */
	class date extends X
	{
		...
		function edit2($time = 0)
		{
			...
			$oy = $sel . ":y'>";
			for($i = date("Y", Core::$core->now) - (! empty($this->args[ 0 ]) ? $this->args[ 0 ] : 5); $i <= date("Y", Core::$core->now) + (! empty($this->args[ 0 ]) ? $this->args[ 0 ] : 5); $i++ )
				$oy .= "<option value='" . $i . "'" . ($i == $d[ 0 ] ? " selected" : "") . ">" . $i . "</option>";
			$oy .= "</select>";
			$om = $sel . ":m'>";
			for($i = 1; $i <= 12; $i++ ) {
				$t = "month" . ($i < 10 ? "0" : "") . $i;
				$om .= "<option value='" . $i . "'" . ($i == intval($d[ 1 ]) ? " selected" : "") . ">" . (! empty(Core::$l[ $t ]) ? Core::$l[ $t ] : ($i < 10 ? "0" : "") . $i) . "</option>";
			}
			$om .= "</select>";
			$od = $sel . ":d'>";
			for($i = 1; $i <= 31; $i++ )
			...
			if($time) {
				$f .= " H:i" . (! empty($this->args[ 3 ]) ? ":s" : "");
				$oh = $sel . ":h'>";
				for($i = 0; $i <= 23; $i++ )
					$oh .= "<option value='" . $i . "'" . ($i == intval($d[ 3 ]) ? " selected" : "") . ">" . (($i < 10 ? "0" : "") . $i) . "</option>";
				$oh .= "</select>";
				$oi = $sel . ":i'>";
				for($i = 0; $i <= 59; $i += (! empty($this->args[ 2 ]) && $this->args[ 2 ] > 0 && $this->args[ 2 ] <= 30 ? $this->args[ 2 ] : 1))
...
/*** for compactness, widely used functions. This makes code a lot smaller ***/
	...
	{
		return " class='" . $a . (! empty($b) && $b != "-" ? " " . $b : "") . "'" . ($c ? " " . $c : "") . ($d ? " name='" . s($d, ".", "_") . "'" : "") . ($e && $e != "-" ? " onchange='" . $e . "'" : "") . (! empty($f[ 0 ]) && $f[ 0 ] > 0 ? " size='" . $f[ 0 ] . "'" : "") . (! empty($f[ 1 ]) && $f[ 1 ] > 0 ? " maxlength='" . $f[ 1 ] . "'" : "");
	}
	function u($a)
	{
		return strlen($a);
