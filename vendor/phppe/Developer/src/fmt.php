...
//			file_put_contents($fn,preg_replace("/data:text\/plain[^\"\']+/","data:text/plain,".str_replace("+","%20",urlencode($out)),file_get_contents($fn)));
			file_put_contents($fn,preg_replace("|/\*core\*/\"data:text\/plain[^\"\']+|","/*core*/\"data:text/plain;base64,".base64_encode($out),preg_replace("|<small data-ver>v[^<]+</small>|","<small data-ver>v".$ver[1]."</small>",file_get_contents($fn))));
		}
	}
}
	...
	foreach($tokens as $n=>$t)
		foreach($t as $type=>$value) {
			...
			if($decompress) {
				...
				if($type==";" || $type=="comments") $out.=$p!=0&&$tokens[$n+1]["const"]!="else"?" ":($type==";"&&(!empty($tokens[$n-1]["keyword"])||!empty($tokens[$n-1]["var"])||!empty($tokens[$n-3]["var"]))&&$funcname==""&&!empty($tokens[$n+1]["comment"])&&empty($tokens[$n+2]["const"])?"":newline($b-(!empty($tokens[$n+1]["}"])?1:0)));
