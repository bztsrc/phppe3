...
function cmp($a,$b){ if($a['prio']+0!=$b['prio']+0)return($a['prio']+0<$b['prio']+0?1:-1);return ($a['name']<$b['name']?-1:1);}
function dumppkg($p,$m,&$json,$l=0){$f=1;foreach($m as $k=>$v) if($k!="prio"){$json.=($f?"":",\n").str_repeat("\t",$l+3)."\"".addslashes($k)."\":";$f=0;if(is_array($v)||is_object($v)) {$json.=isset($v[0])?"[\n":"{\n";dumppkg($k,$v,$json,$l+1);$json.="\n".str_repeat("\t",$l+3).(isset($v[0])?"]":"}");} else $json.="\"".addslashes($v)."\"";}}
...
foreach($d as $v) {
	...
	foreach($m as $K=>$V) if(substr($K,0,5)=="name_"||substr($K,0,12)=="description_") $packages[$v][$K]=$V;
